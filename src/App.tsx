  ...
  return (
    <div className="kb-shell">
      ...
      <section className="main-layout">
        <aside className="category-panel glass"><div className="panel-title"><Search size={15} /> 分类导航</div><div className="category-list">{(snapshot?.categories ?? []).map((category) => <button type="button" key={category.id} className={category.id === activeCategoryId ? 'category-item active' : 'category-item'} onClick={() => setManualCategoryId(category.id)}><div><strong>{category.name}</strong><small>{category.quotes.length}/10</small></div><em>天津 {category.quotes.filter((quote) => quote.isTianjinPriority).length}</em></button>)}</div>{activeCategory ? <div className="category-brief-card"><div className="category-brief-head"><strong>{activeCategory.name}速览</strong><span>补足导航下方信息层</span></div><div className="category-brief-stats"><div><small>细分品类</small><strong>{activeCategory.detail.subcategories.length}</strong></div><div><small>有效报价</small><strong>{activeCategory.quotes.length}</strong></div><div><small>法规动态</small><strong>{activeCategory.detail.regulationUpdates.length}</strong></div><div><small>异动新闻</small><strong>{categoryAlertNews.length}</strong></div></div><div className="category-brief-note"><span>当前重点</span><p>{activeCategory.detail.painPoints[0] ?? '当前未生成行业痛点摘要。'}</p></div>{latestRegulationUpdate ? <div className="category-brief-note"><span>最新官方动态</span><a href={latestRegulationUpdate.link} target="_blank" rel="noreferrer">{latestRegulationUpdate.title}</a></div> : null}</div> : null}</aside>
