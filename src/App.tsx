          ...
          <div className="news-ribbon-window">
            <div className="news-ribbon-track">
              {[...tickerQuotes, ...tickerQuotes].map((item, index) => (
                <a key={`${item.id}-${index}`} href={item.sourceUrl} target="_blank" rel="noreferrer" className="ticker-chip price-ticker-chip" title={`${item.categoryName} ${item.primary}`}>
          ...
          {activeCategory ? <>
            <section className="glass section-card detail-overview-card">
              ...
              <div className="detail-overview-grid">
                ...
                <div className="detail-overview-stats"><div><small>报价条数</small><strong>{activeCategory.quotes.length}</strong></div><div><small>异动新闻</small><strong>{categoryAlertNews.length}</strong></div><div><small>官方动态</small><strong>{activeCategory.detail.regulationUpdates.length}</strong></div><div><small>细分品类</small><strong>{activeCategory.detail.subcategories.length}</strong></div></div>
              </div>
            </section>
